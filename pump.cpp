...
void myServer(EthernetClient client, String _timer) {
  if (client) {
    String data = "", keyVal = "";
    String myArray[2];
    int myIndex = 0;
    while (client.connected()) {
      ...
      client.println("<body>");
      client.println(
          "<style>*{margin:10px auto;text-align:center;}.r{color:rgba(255,0,0,0.6);}.g{color:rgba(0,255,0,0.6);}</style>");
      client.println("<body>");
...
void loop() {
  ...
  String currTimer = String(mm < 10 ? (String("0") + String(mm)) : String(mm)) + String(":") +
                     String(ss < 10 ? (String("0") + String(ss)) : String(ss));
